...
class LLMSharpAgent:
    ...
    def analyze_fixture(
        self, match_data: dict[str, Any], edge_info: dict[str, Any]
    ) -> str:
        """
...
"""
    ...
    def _query_anything_llm(self, prompt: str) -> str | None:
        try:
            headers = {
            ...
            payload = {
                ...
                "messages": [
                    {
                        "role": "system",
                        "content": "You are a professional quantitative sports bettor.",
                    },
            ...
            res = requests.post(
                ANYTHING_LLM_URL, json=payload, headers=headers, timeout=4
            )
            if res.status_code == 200:
                data = res.json()
    ...
    def _algorithmic_commentary(
        ...
        if edge > 10.0:
            return (
                f"[POTVRĐENO]: Značajan diskorak u kvoti za {pick}. xG diferencijal favorizuje "
                f"ovu opciju sa izraženom matematičkom vrednošću (+{edge:.1f}% edge)."
            )
        elif edge >= 5.0:
            return (
                f"[POTVRĐENO]: Umereni +EV (+{edge:.1f}%) na meču {h_team} vs {a_team}. "
                f"Preporučuje se striktno poštovanje Half-Kelly uloga bez preteranog izlaganja."
            )
        else:
            return "[OPREZ]: Margina je ispod standardnog praga sigurnosti. Preporučuje se posmatranje uživo."
