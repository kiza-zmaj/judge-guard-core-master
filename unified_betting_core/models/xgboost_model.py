...
class TimeDecayModel:
    ...
    def calculate_form_rating(self, team_name: str, historical_df: pd.DataFrame) -> Dict[str, float]:
        """
    ...
    def adjust_match_xg(self, home_team: str, away_team: str, raw_home_xg: float, raw_away_xg: float, hist_df: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        adj_home_xg = (raw_home_xg * 0.6) + (home_form["attack_rating"] * 0.2) + (away_form["defense_rating"] * 0.2)
        adj_away_xg = (raw_away_xg * 0.6) + (away_form["attack_rating"] * 0.2) + (home_form["defense_rating"] * 0.2)
