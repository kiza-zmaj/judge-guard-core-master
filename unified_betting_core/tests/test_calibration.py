...
class TestModelCalibration(unittest.TestCase):
    ...
    def test_evaluate_gate_rejects_empty(self):
        ...
        self.assertTrue(any("Insufficient OOS sample" in r for r in verdict.failure_reasons))
        self.assertTrue(any("ECE could not be computed" in r for r in verdict.failure_reasons))
    ...
    def test_evaluate_gate_rejects_few_samples(self):
        ...
        verdict = ModelCalibration.evaluate_out_of_sample_gate(preds, actuals, min_sample_size=100)
        self.assertFalse(verdict.passed)
        self.assertEqual(verdict.status, "CALIBRATION_FAILED")
        self.assertTrue(any("Insufficient OOS sample" in r for r in verdict.failure_reasons))
