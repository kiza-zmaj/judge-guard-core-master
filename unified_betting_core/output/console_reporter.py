...
"""
    ...
    @staticmethod
    def print_11_step_pipeline_audit(
        audited_bets: list[dict[str, Any]], bankroll: float
    ):
        """
        ...
        def _render_subtable(title: str, bets: list[dict[str, Any]]):
            ...
            for b in bets:
                ...
                cal_ev = (
                    f"{b.get('calibrated_ev_pct', b.get('true_ev_pct', 0.0)):+.1f}%"
                )
        ...
        if live_bets:
            _render_subtable(
                "🔴 [UŽIVO U TOKU: IN-PLAY REAL-TIME PREDVIĐANJA]", live_bets
            )

        if upcoming_bets:
            _render_subtable(
                "⏳ [PREDSTOJEĆI DANAŠNJI MEČEVI: KONSENZUS & MODEL]", upcoming_bets
            )
    ...
    def print_verified_bets(verified_bets: list[dict[str, Any]], bankroll: float):
        ...
        for b in verified_bets:
            ...
            print(
                f"{match:<30} | {bet_on:<5} | {odds:<6} | {prob:<8} | {edge:<9} | {stake:<10} | {status:<18}"
            )
    ...
    def print_empirical_evidence_report(report_data: dict[str, Any]):
        ...
        print(
            f"      Izvor mečeva i kvota:          {data_info.get('odds_source', 'football-data.co.uk + Pinnacle')}"
        )
        print(
            f"      Period evaluacije:             {data_info.get('date_range', '2022-08-05 do 2025-05-25 (3 sezone)')}"
        )
        print(
            f"      Ukupan broj realnih mečeva:    {data_info.get('total_matches', 1140)}"
        )
        print(
            f"      Out-of-sample mečevi:          {data_info.get('oos_matches', 760)}"
        )
        print(
            f"      Status integriteta podataka:   {data_info.get('data_quality_state', 'LIVE/VERIFIED')}"
        )
        print(
        ...
        print(
            f"      Brier Score (Multi-class):     {calib.get('brier_score', 0):.4f} (Idealno: 0.0, Baseline: 0.667)"
        )
        ...
        print(
            f"      Prosečan Raw CLV vs Pinnacle:  {clv.get('avg_raw_clv_pct', 0):+.2f}%"
        )
        print(
            f"      Median Raw CLV vs Pinnacle:    {clv.get('median_raw_clv_pct', 0):+.2f}%"
        )
        print(
        ...
        print(
            f"      Početni bankroll:              €{pnl.get('initial_bankroll', 1000):,.2f}"
        )
        print(
            f"      Završni bankroll:              €{pnl.get('current_bankroll', 1000):,.2f}"
        )
        print(f"      Ukupno odobrenih opklada:      {pnl.get('total_bets', 0)}")
        print(
            f"      Realizovani Turnover:          €{pnl.get('total_staked', 0):,.2f}"
        )
        print(f"      Neto realizovani profit:       €{pnl.get('net_profit', 0):+,.2f}")
        print(
            f"      Realizovani ROI / Yield:       {pnl.get('roi_yield_pct', 0):+.2f}%"
        )
        print(
            f"      Maksimalni Drawdown:           {pnl.get('max_drawdown_pct', 0):.1f}%"
        )
        ...
        print(
            f"      Blokirano CALIBRATION_FAILED:  {rej.get('CALIBRATION_FAILED', 0)}"
        )
        print(
            f"      Blokirano INSUFFICIENT_EVID.:  {rej.get('INSUFFICIENT_EVIDENCE', 0)}"
        )
